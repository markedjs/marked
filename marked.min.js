...
`}strong(e){return`<strong>${e}</strong>`}em(e){return`<em>${e}</em>`}codespan(e){return`<code>${e}</code>`}br(){return"<br>"}del(e){return`<del>${e}</del>`}link(e,t,n){if(null===(e=u(e)))return n;let r='<a href="'+e+'"';return t&&(r+=' title="'+t+'"'),r+=">"+n+"</a>"}image(e,t,n){if(null===(e=u(e)))return n;let r=`<img src="${e}" alt="${n}"`;return t&&(r+=` title="${t}"`),r+=">"}text(e){return e}}class y{strong(e){return e}em(e){return e}codespan(e){return e}del(e){return e}html(e){return e}text(e){return e}link(e,t,n){return""+n}image(e,t,n){return""+n}br(){return""}}class ${options;renderer;textRenderer;constructor(e){this.options=e||t.defaults,this.options.renderer=this.options.renderer||new _,this.renderer=this.options.renderer,this.renderer.options=this.options,this.textRenderer=new y}static parse(e,t){return new $(t).parse(e)}static parseInline(e,t){return new $(t).parseInline(e)}parse(e,t=!0){let n="",r,s,i,l,a,o,c,h,p,u,g,k,f,d,x,m,b,w,_;var y=e.length;for(r=0;r<y;r++)if(u=e[r],this.options.extensions&&this.options.extensions.renderers&&this.options.extensions.renderers[u.type]&&(!1!==(_=this.options.extensions.renderers[u.type].call({parser:this},u))||!["space","hr","heading","code","table","blockquote","list","html","paragraph","text"].includes(u.type)))n+=_||"";else switch(u.type){case"space":continue;case"hr":n+=this.renderer.hr();continue;case"heading":n+=this.renderer.heading(this.parseInline(u.tokens),u.depth,this.parseInline(u.tokens,this.textRenderer).replace(z,(e,t)=>"colon"===(t=t.toLowerCase())?":":"#"===t.charAt(0)?"x"===t.charAt(1)?String.fromCharCode(parseInt(t.substring(2),16)):String.fromCharCode(+t.substring(1)):""));continue;case"code":n+=this.renderer.code(u.text,u.lang,!!u.escaped);continue;case"table":for(h="",c="",l=u.header.length,s=0;s<l;s++)c+=this.renderer.tablecell(this.parseInline(u.header[s].tokens),{header:!0,align:u.align[s]});for(h+=this.renderer.tablerow(c),p="",l=u.rows.length,s=0;s<l;s++){for(o=u.rows[s],c="",a=o.length,i=0;i<a;i++)c+=this.renderer.tablecell(this.parseInline(o[i].tokens),{header:!1,align:u.align[i]});p+=this.renderer.tablerow(c)}n+=this.renderer.table(h,p);continue;case"blockquote":p=this.parse(u.tokens),n+=this.renderer.blockquote(p);continue;case"list":for(g=u.ordered,k=u.start,f=u.loose,l=u.items.length,p="",s=0;s<l;s++)m=(x=u.items[s]).checked,b=x.task,d="",x.task&&(w=this.renderer.checkbox(!!m),f?0<x.tokens.length&&"paragraph"===x.tokens[0].type?(x.tokens[0].text=w+" "+x.tokens[0].text,x.tokens[0].tokens&&0<x.tokens[0].tokens.length&&"text"===x.tokens[0].tokens[0].type&&(x.tokens[0].tokens[0].text=w+" "+x.tokens[0].tokens[0].text)):x.tokens.unshift({type:"text",text:w}):d+=w),d+=this.parse(x.tokens,f),p+=this.renderer.listitem(d,b,!!m);n+=this.renderer.list(p,g,k);continue;case"html":n+=this.renderer.html(u.text,u.block);continue;case"paragraph":n+=this.renderer.paragraph(this.parseInline(u.tokens));continue;case"text":for(p=u.tokens?this.parseInline(u.tokens):u.text;r+1<y&&"text"===e[r+1].type;)u=e[++r],p+="\n"+(u.tokens?this.parseInline(u.tokens):u.text);n+=t?this.renderer.paragraph(p):p;continue;default:var $='Token with "'+u.type+'" type was not found.';if(this.options.silent)return console.error($),"";throw new Error($)}return n}parseInline(e,t){t=t||this.renderer;let n="",r,s,i;var l=e.length;for(r=0;r<l;r++)if(s=e[r],this.options.extensions&&this.options.extensions.renderers&&this.options.extensions.renderers[s.type]&&(!1!==(i=this.options.extensions.renderers[s.type].call({parser:this},s))||!["escape","html","link","image","strong","em","codespan","br","del","text"].includes(s.type)))n+=i||"";else switch(s.type){case"escape":n+=t.text(s.text);break;case"html":n+=t.html(s.text);break;case"link":n+=t.link(s.href,s.title,this.parseInline(s.tokens,t));break;case"image":n+=t.image(s.href,s.title,s.text);break;case"strong":n+=t.strong(this.parseInline(s.tokens,t));break;case"em":n+=t.em(this.parseInline(s.tokens,t));break;case"codespan":n+=t.codespan(s.text);break;case"br":n+=t.br();break;case"del":n+=t.del(this.parseInline(s.tokens,t));break;case"text":n+=t.text(s.text);break;default:var a='Token with "'+s.type+'" type was not found.';if(this.options.silent)return console.error(a),"";throw new Error(a)}return n}}class R{options;constructor(e){this.options=e||t.defaults}static passThroughHooks=new Set(["preprocess","postprocess"]);preprocess(e){return e}postprocess(e){return e}}class v{defaults=e();options=this.setOptions;parse=this.#parseMarkdown(w.lex,$.parse);parseInline=this.#parseMarkdown(w.lexInline,$.parseInline);Parser=$;parser=$.parse;Renderer=_;TextRenderer=y;Lexer=w;lexer=w.lex;Tokenizer=b;Hooks=R;constructor(...e){this.use(...e)}walkTokens(e,t){let n=[];for(const r of e)switch(n=n.concat(t.call(this,r)),r.type){case"table":for(const s of r.header)n=n.concat(this.walkTokens(s.tokens,t));for(const i of r.rows)for(const l of i)n=n.concat(this.walkTokens(l.tokens,t));break;case"list":n=n.concat(this.walkTokens(r.items,t));break;default:this.defaults.extensions&&this.defaults.extensions.childTokens&&this.defaults.extensions.childTokens[r.type]?this.defaults.extensions.childTokens[r.type].forEach(e=>{n=n.concat(this.walkTokens(r[e],t))}):r.tokens&&(n=n.concat(this.walkTokens(r.tokens,t)))}return n}use(...e){const m=this.defaults.extensions||{renderers:{},childTokens:{}};return e.forEach(n=>{var e={...n};if(e.async=this.defaults.async||e.async||!1,n.extensions&&(n.extensions.forEach(n=>{if(!n.name)throw new Error("extension name required");if("renderer"in n){const r=m.renderers[n.name];r?m.renderers[n.name]=function(...e){let t=n.renderer.apply(this,e);return t=!1===t?r.apply(this,e):t}:m.renderers[n.name]=n.renderer}if("tokenizer"in n){if(!n.level||"block"!==n.level&&"inline"!==n.level)throw new Error("extension level must be 'block' or 'inline'");m[n.level]?m[n.level].unshift(n.tokenizer):m[n.level]=[n.tokenizer],n.start&&("block"===n.level?m.startBlock?m.startBlock.push(n.start):m.startBlock=[n.start]:"inline"===n.level&&(m.startInline?m.startInline.push(n.start):m.startInline=[n.start]))}"childTokens"in n&&n.childTokens&&(m.childTokens[n.name]=n.childTokens)}),e.extensions=m),n.renderer){const i=this.defaults.renderer||new _(this.defaults);for(const l in n.renderer){const a=n.renderer[l];var t=l;const o=i[t];i[t]=(...e)=>{let t=a.apply(i,e);return(t=!1===t?o.apply(i,e):t)||""}}e.renderer=i}if(n.tokenizer){const c=this.defaults.tokenizer||new b(this.defaults);for(const h in n.tokenizer){const p=n.tokenizer[h];var r=h;const u=c[r];c[r]=(...e)=>{let t=p.apply(c,e);return t=!1===t?u.apply(c,e):t}}e.tokenizer=c}if(n.hooks){const g=this.defaults.hooks||new R;for(const k in n.hooks){const f=n.hooks[k];var s=k;const d=g[s];R.passThroughHooks.has(k)?g[s]=e=>{return this.defaults.async?Promise.resolve(f.call(g,e)).then(e=>d.call(g,e)):(e=f.call(g,e),d.call(g,e))}:g[s]=(...e)=>{let t=f.apply(g,e);return t=!1===t?d.apply(g,e):t}}e.hooks=g}if(n.walkTokens){const x=this.defaults.walkTokens;e.walkTokens=function(e){let t=[];return t.push(n.walkTokens.call(this,e)),t=x?t.concat(x.call(this,e)):t}}this.defaults={...this.defaults,...e}}),this}setOptions(e){return this.defaults={...this.defaults,...e},this}#parseMarkdown(i,l){return(t,n)=>{n={...n};const r={...this.defaults,...n};n=this.#onError(!!r.silent,!!r.async);if(null==t)return n(new Error("marked(): input parameter is undefined or null"));if("string"!=typeof t)return n(new Error("marked(): input parameter is of type "+Object.prototype.toString.call(t)+", string expected"));if(r.hooks&&(r.hooks.options=r),r.async)return Promise.resolve(r.hooks?r.hooks.preprocess(t):t).then(e=>i(e,r)).then(e=>r.walkTokens?Promise.all(this.walkTokens(e,r.walkTokens)).then(()=>e):e).then(e=>l(e,r)).then(e=>r.hooks?r.hooks.postprocess(e):e).catch(n);try{r.hooks&&(t=r.hooks.preprocess(t));var s=i(t,r);r.walkTokens&&this.walkTokens(s,r.walkTokens);let e=l(s,r);return e=r.hooks?r.hooks.postprocess(e):e}catch(e){return n(e)}}}#onError(n,r){return e=>{var t;if(e.message+="\nPlease report this to https://github.com/markedjs/marked.",n)return t="<p>An error occurred:</p><pre>"+c(e.message+"",!0)+"</pre>",r?Promise.resolve(t):t;if(r)return Promise.reject(e);throw e}}}const S=new v;function T(e,t){return S.parse(e,t)}T.options=T.setOptions=function(e){return S.setOptions(e),n(T.defaults=S.defaults),T},T.getDefaults=e,T.defaults=t.defaults,T.use=function(...e){return S.use(...e),n(T.defaults=S.defaults),T},T.walkTokens=function(e,t){return S.walkTokens(e,t)},T.parseInline=S.parseInline,T.Parser=$,T.parser=$.parse,T.Renderer=_,T.TextRenderer=y,T.Lexer=w,T.lexer=w.lex,T.Tokenizer=b,T.Hooks=R;var g=(T.parse=T).options,A=T.setOptions,I=T.use,Z=T.walkTokens,q=T.parseInline,E=T,L=$.parse,D=w.lex;t.Hooks=R,t.Lexer=w,t.Marked=v,t.Parser=$,t.Renderer=_,t.TextRenderer=y,t.Tokenizer=b,t.getDefaults=e,t.lexer=D,t.marked=T,t.options=g,t.parse=E,t.parseInline=q,t.parser=L,t.setOptions=A,t.use=I,t.walkTokens=Z});