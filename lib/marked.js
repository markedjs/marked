...
Renderer.prototype.tablecell = function(content, flags) {
  ...
  var tag = flags.align ? '<' + type + ' align="' + flags.align + '">' : '<th>';
  ...
  switch (this.token.type) {
    ...
    case 'table': {
      ...
      for (i = 0; i < this.token.header.length; i++) {
        ...
        cell = renderer.tablecell(this.inline.output(this.token.header[i]), flags);
        // render row
      ...
      for (i = 0; i < this.token.cells.length; i++) {
        ...
        for (j = 0; j < row.length; j++) {
          flags = {header: false, align: this.token.align[i]};
          cell += renderer.tablecell(this.inline.output(row[j]), flags);
        }
    ...
    case 'list_start': {
      ...
      return renderer.list(body, this.token.ordered);
    }
