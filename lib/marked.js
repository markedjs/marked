...
var inline = {
  ...
  text: /^[\s\S]+?(?=[\\<!\[_*`\:]| {2,}\n|$)/,
  emoji: /^\:([\w|\+|\-]{2,}?)\:/,
  //String used to match against valid emojis
  ...
  while (src) {
    ...
    if(this.options.emojis) {
      if (cap = this.rules.emoji.exec(src)) {
        src = src.substring(cap[0].length);
        //There is a but que doesn't allow search, to search a '+' string
        if ( inline.emojis.search(cap[0]) !== -1 || cap[0] === ':+1:') {
          out += '<img class="emoji" src="'
            + this.options.emojisPath
            + (cap[1])
            + '.png"/>';  
        } else {
          out += cap[0];
