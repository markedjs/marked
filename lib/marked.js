...
InlineLexer.prototype.output = function(src) {
  ...
  var promise = this.options.promise;
  ...
  if (promiseSupport && promise) return out;
  return out.join(''); 
};
...
Parser.prototype.parse = function(src) {
  ...
  if (promiseSupport){
    ...
    throw new Error("Promise not support!");
  }
};
...
Parser.prototype.tok = function() {
  ...
  switch (this.token.type) {
    ...
    case 'blockquote_start': {
      ...
      if (promiseSupport && promise) {
        var bodyblocks = [];
        while (this.next().type !== 'blockquote_end') {
          bodyblocks.push(this.tok());
        }
        return this.renderer.blockquote(bodyblocks);
      } else {
        var body = '';
        while (this.next().type !== 'blockquote_end') {
          body += this.tok();
...
function marked(src, opt, callback) {
  ...
  try {
    ...
    if (promiseSupport && opt && opt.promise) {
      return (function(src, opt) {
        return new Promise(function(resolve, reject) {
          try {
            resolve(Lexer.lex(src, opt));
          } catch(e) {
            reject(e);
          }
        })
